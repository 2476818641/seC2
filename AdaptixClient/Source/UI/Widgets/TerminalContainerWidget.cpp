...
void TerminalTab::createUI()
{
    ...
    if (this->agent && this->agent->data.Os == OS_WINDOWS) {
        programComboBox->addItem(tr("Cmd"));
        programComboBox->addItem(tr("Powershell"));
        programInput->setText("C:\\Windows\\System32\\cmd.exe");
    }
    else if (this->agent && this->agent->data.Os == OS_LINUX){
        programComboBox->addItem(tr("Shell"));
        programComboBox->addItem(tr("Bash"));
        programInput->setText("/bin/sh");
    }
    else {
        programComboBox->addItem(tr("ZSH"));
        programComboBox->addItem(tr("Shell"));
        programComboBox->addItem(tr("Bash"));
        programInput->setText("/bin/zsh");
    }
    programComboBox->addItem(tr("Custom program"));
...
void TerminalTab::setStatus(const QString &text)
{
    ...
    if (text == tr("Stopped")) {
        programInput->setEnabled(programComboBox->currentText() == tr("Custom program"));
...
void TerminalTab::changeEvent(QEvent *event)
{
    if (event->type() == QEvent::LanguageChange) {
        keytabLabel->setText(tr("Keytab:"));
        statusDescLabel->setText(tr("status:"));
        statusLabel->setText(tr("Stopped"));
        smartOutputCheckBox->setText(tr("Smart Output"));
        startButton->setToolTip(tr("Start terminal"));
        stopButton->setToolTip(tr("Stop terminal"));
    }
...
TerminalContainerWidget::TerminalContainerWidget(Agent* a, AdaptixWidget* w, TerminalMode mode) : DockTab(QString("%1 [%2]").arg(mode == TerminalModeShell ? tr("Shell") : tr("Terminal")).arg(a->data.Id), w->GetProfile()->GetProject())
{
    this->agent = a;
    this->adaptixWidget = w;
    this->terminalMode = mode;
...
void TerminalContainerWidget::addNewTerminal()
{
    ...
    QString tabName = (terminalMode == TerminalModeShell) ? QString(tr("Shell %1")).arg(tabCounter) : QString(tr("Term %1")).arg(tabCounter);
...
void TerminalContainerWidget::changeEvent(QEvent *event)
{
    if (event->type() == QEvent::LanguageChange) {
        QString title = QString("%1 [%2]").arg(terminalMode == TerminalModeShell ? tr("Shell") : tr("Terminal")).arg(agent->data.Id);
        setWindowTitle(title);
    }
    DockTab::changeEvent(event);
}
